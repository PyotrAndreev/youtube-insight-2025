...
def get_videos_from_channel(title):
    info_about_channel = []
    chanel_video_titles = []
    channel_views = []
    channel_likes = []
    ...
    channel_comments_amount = []
    len_comm_by_video = 0
    videos = videoRepository.get_by_channel_title(title)
    print(videos)
    for video in videos:
        if video is not None:
            for (comment) in video.comments:
            ...
            if video.comment_count is None:
                ...
                comm_amount=video.comment_count
            if video.title not in chanel_video_titles:
                chanel_video_titles.append(video.title)
                channel_comments_length.append(len_comm_by_video)
                channel_comments_amount.append(comm_amount)
                channel_views.append(video.view_count)
                channel_likes.append(video.like_count)
            else:
                ind = chanel_video_titles.index(video.title)
                channel_comments_length[ind] += len_comm_by_video
                channel_comments_amount[ind] += comm_amount
                channel_views[ind] += video.view_count
                channel_likes[ind] += video.like_count
    info_about_channel.append(chanel_video_titles)
    info_about_channel.append(channel_views)
    info_about_channel.append(channel_likes)
    info_about_channel.append(channel_comments_length)
    info_about_channel.append(channel_comments_amount)
...
def get_videos_from_same_channels(manual_category):
    info_about_channel = []
    chanel_video_titles = []
    channel_views = []
    channel_likes = []
    ...
    for video in videos:
        if video is not None:
            playlist = playlistRepository.get_by_id(video.playlist_id)
            year = int(str(video.published_at).split("-")[0])
            for (comment) in video.comments:
                len_comm = len(comment.text)
                len_comm_by_video += len_comm
            if video.comment_count is None:
                ...
                comm_amount=video.comment_count
            if video.channel_title not in chanel_video_titles:
                chanel_video_titles.append(video.channel_title)
                channel_comments_length.append(len_comm_by_video)
                channel_comments_amount.append(comm_amount)
                channel_views.append(video.view_count)
                channel_likes.append(video.like_count)
                channel_date.append(video.published_at)
            else:
                ind = chanel_video_titles.index(video.channel_title)
                channel_comments_length[ind] += len_comm_by_video
                channel_comments_amount[ind] += comm_amount
                channel_views[ind] += video.view_count
                channel_likes[ind] += video.like_count
    ...
    info_about_channel.append(channel_comments_length)
    info_about_channel.append(channel_comments_amount)
    #info_about_channel.append(channel_date)
    return info_about_channel

#print(get_videos_from_channel_by_years('SciOne'))